...
Bank switching and MMU-aware debugging
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

With modern machines, typically the logical address space is far larger than the actual
physical RAM installed on the machine. But with vintage machines, it can easily be
the opposite. With an 8/16-bit CPU that is capable of addressing only a 64K logical
address space, there may well be more than 64K of physical RAM installed on the machine.
The CoCo 3 is a good example of just such a machine. 
The user may issue debugger commands to help MAME know when code is being swapped in
and out of a range of logical addresses. If you automate the issuing of these commands
with a LUA script (see below), the end result is an intelligent debugging experience
where the source and symbols automatically change as physical RAM gets swapped in
and out of the logical address space.

**Background - Multiple MDIs**

Typically you provide a single MAME Debugging Information File (:ref:`MDI <srcdbg_mdi>`)
on the MAME command line, and you're good. Even in cases when you have multiple assembly
files which you assemble to obj files, and then link together, you would still use the
single combined MDI file generated by the linker, and specify that on the MAME command line

However, there are scenarios where the same "logical" addresses (i.e., the addresses used
by the CPU to refer to an area of memory) might have different blocks of physical RAM
mapped to them at different times. And sometimes that physical RAM contains executable
code. The MAME debugger needs to know which code is mapped in at a given time so it
knows which source to show. The debugger does not do complex analysis of the code
it's debugging, and does not understand the different hardware nuances of the
emulated machines, so it needs your help to know which source level debugging
information to consult at any given time.

This is done through the use of multiple MDI files. Rather than specifying
a single MDI file path on the MAME command line, specify a semicolon-separated
list of all the MDI files that comprise the program you are debugging. So
long as blocks of code that may overlap in the logical address space appear
in different MDI files, you can tell the debugger--during the course
of program execution--which MDI files to enable or disable at any given time.

**Example**

Suppose your assembler generates three intermediate object files, each
representing a block of code that is mapped into the logical address
space at a given time.  Assume their respective MDIs are:

::

    c:\project\BlocksA.mdi
    c:\project\BlocksB.mdi
    c:\project\BlocksC.mdi

and the combined MDI generated by the assembler or linker is:

::

    c:\project\BlocksALL.mdi

Normally, you would only specify ``-src_debug_info c:\project\BlocksALL.mdi``
on the MAME command-line.  But in order to take advantange of "MMU-aware"
debugging, you'd want to separate code that could, at different times, be
swapped in to the same address ranges.  To do so, you'd specify the intermediate MDI
files instead: ``-src_debug_info c:\project\BlocksA.mdi;c:\project\BlocksBL.mdi;c:\project\BlocksB.mdi``.

From the debugger console window, you can now use the
:ref:`sdlist <_debugger-command-sdlist>`, :ref:`sdenable <_debugger-command-sdenable>`,
and :ref:`sddisable <_debugger-command-sddisable>` commands to choose which
subset of MDIs to enable at any given time.  Doing this dynamically adjusts which source
code files are available to view in the source-level debugging window.
You can automate this process using a customized LUA script which responds to writes to
the emulated computer's MMI addresses, and then issues the appropriate 
:ref:`sdenable <_debugger-command-sdenable>` and
:ref:`sddisable <_debugger-command-sddisable>` commands to ensure only
the correct MDIs are enabled at any given time.
